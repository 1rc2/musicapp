    ...
    private android.webkit.WebResourceResponse interceptMusicRequest(String url) {
        ...
        if (url != null && url.startsWith("https://musicflow.local/music/")) {
            try {
                ...
                Log.d("MusicFlow", "拦截音乐请求: " + url + " -> 文件名: " + fname);
                File musicDir = new File(getFilesDir(), "music");
                File file = new File(musicDir, fname);
                Log.d("MusicFlow", "查找文件: " + file.getAbsolutePath() + " 存在=" + file.exists());
                if (file.exists() && file.canRead()) {
                    ...
                    Log.d("MusicFlow", "返回音频流: " + file.length() + " bytes");
                    return new android.webkit.WebResourceResponse(
                        "audio/mpeg", "UTF-8", 200, "OK", headers, is);
                } else {
                    Log.e("MusicFlow", "文件不存在或不可读: " + file.getAbsolutePath());
                }
            } catch (Exception e) {
                Log.e("MusicFlow", "拦截音乐请求异常", e);
            }
        }
        // 拦截自定义封面图片
    ...
    class MusicBridge {
        ...
        public void downloadOnlineSong(String songUrl, String filename) {
            new Thread(() -> {
                HttpURLConnection conn = null;
                try {
                    Log.d("MusicFlow", "开始下载歌曲: " + songUrl + " -> " + filename);
                    ...
                    Log.d("MusicFlow", "下载响应码: " + responseCode);
                    if (responseCode != 200) {
                        throw new Exception("服务器返回: " + responseCode);
                    }

                    int totalSize = conn.getContentLength();
                    Log.d("MusicFlow", "文件大小: " + totalSize);
                    InputStream is = conn.getInputStream();
                    ...
                    // 重名文件自动加序号，避免覆盖已下载的歌曲
                    File outFile = new File(musicDir, decodedFilename);
                    int counter = 1;
                    while (outFile.exists()) {
                        int dotIdx = decodedFilename.lastIndexOf('.');
                        String newName = (dotIdx > 0)
                            ? decodedFilename.substring(0, dotIdx) + "_" + counter + decodedFilename.substring(dotIdx)
                            : decodedFilename + "_" + counter;
                        outFile = new File(musicDir, newName);
                        counter++;
                    }
                    final String actualFilename = outFile.getName();
                    Log.d("MusicFlow", "保存到: " + outFile.getAbsolutePath());
                    FileOutputStream fos = new FileOutputStream(outFile);
                    ...
                    Log.d("MusicFlow", "下载完成: " + path + " 大小=" + size + " bytes");
                    if (size == 0) {
                        throw new Exception("下载文件为空");
                    }
                    // 使用自定义 URL scheme，通过 shouldInterceptRequest 提供文件
                    // 文件名需要 URL 编码以支持中文，使用实际保存的文件名（可能含重名序号）
                    final String encodedName = java.net.URLEncoder.encode(actualFilename, "UTF-8")
                        .replace("+", "%20");
                    final String playUrl = "https://musicflow.local/music/" + encodedName;
                    Log.d("MusicFlow", "播放URL: " + playUrl);
        ...
        /** 同步获取在线歌曲真实播放地址（可能阻塞，建议用 getOnlineSongUrlAsync） */
        @JavascriptInterface
        public String getOnlineSongUrl(String songId) {
            return getOnlineSongUrlInternal(songId);
        }

        /** 异步获取在线歌曲真实播放地址，通过 JS 回调返回，不阻塞 WebView 主线程 */
        @JavascriptInterface
        ...
        /** 调用网易云 API 获取真实播放地址 */
        private String getOnlineSongUrlInternal(String songId) {
            HttpURLConnection conn = null;
            try {
                String apiUrl = "https://music.163.com/api/song/url?id=" + songId;
                ...
                if (conn.getResponseCode() == 200) {
                    ...
                    if (data != null && data.length() > 0) {
                        ...
                        String realUrl = song.optString("url", "");
                        if (realUrl != null && !realUrl.isEmpty() && !"null".equals(realUrl)) {
                            return realUrl;
                        }
                    }
                }
            } catch (Exception e) {
                Log.e("MusicFlow", "获取在线歌曲URL失败: " + songId, e);
            } finally {
                if (conn != null) conn.disconnect();
            }
            return "";
